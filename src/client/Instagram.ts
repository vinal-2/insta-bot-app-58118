...
import { IGpassword, IGusername, TARGET_POST_URLS } from "../secret";
...
async function runInstagram(): Promise<void> {
    ...
    if (TARGET_POST_URLS.length === 0) {
        logger.info("No target post URLs found in .env. Skipping post interaction.");
    } else {
        // Interact with each post from the list of URLs
        for (const url of TARGET_POST_URLS) {
            await interactWithPost(page, url);
        }
    }
...
async function interactWithPost(page: any, url: string) {
    ...
    while (postIndex <= maxPosts) {
        try {
            ...
            const postExist = await page.$("article");
            if (!postExist) {
                logger.warn(`Post not found: ${url}`);
                return; // Skip to the next URL
            }
            //wait 5 seconds for the page to load properly.
